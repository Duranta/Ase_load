...
void cObjMap::Load(char* szFullPath, D3DXMATRIXA16* pmat)
{
	...
	while (true)
	{
		...
		{
			int aIndex[3];
			sscanf(szTemp, "%*s %d/%*d/%*d %d/%*d/%*d %d/%*d/%*d", &aIndex[0], &aIndex[1], &aIndex[2]);
...
bool cObjMap::GetHeight(IN const float& x, OUT float& y, IN const float& z)
{
	...
	{
		if (D3DXIntersectTri(&m_vecVertex[i], &m_vecVertex[i + 1], &m_vecVertex[i + 2], &vRayOrig, &vRayDir, &u, &v, &d))
		{
			y = 1000 - d;
			return true;
		}
